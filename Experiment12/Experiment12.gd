...
func _ready():
	...
	mini_map.set_map_scale(mini_map.get_rect().size.x / map_rect.size.x)
	mini_map.set_map_view(viewport)
...
func _process(_delta):
	...
	if update:
		...
		var mini: Rect2 = Rect2(-offset.x, -offset.y, viewport.end.x, viewport.end.y)
		mini_map.set_map_view(mini)
